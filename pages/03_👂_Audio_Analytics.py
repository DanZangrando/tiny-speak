...
def analyze_real_audio_metrics(audio_input, is_path=False):
    """
    Análisis completo de métricas de audio reales usando librosa
    audio_input: puede ser base64 string o path al archivo
    is_path: booleano indicando si audio_input es un path
    """
    if not audio_input:
    ...
    try:
        ...
        if is_path:
            ...
            if os.path.exists(audio_input):
                ...
                st.warning(f"Archivo no encontrado: {audio_input}")
                return get_default_metrics()
        else:
    ...
    for palabra, variaciones in samples.items():
        for i, variacion in enumerate(variaciones):
            ...
            if 'file_path' in variacion:
                ...
                metrics = analyze_real_audio_metrics(str(file_path), is_path=True)
            else:
                # Usar base64 (legacy)
                audio_base64 = variacion.get('audio_base64', '')
                metrics = analyze_real_audio_metrics(audio_base64, is_path=False)
...
def mostrar_analisis_dataset():
    ...
    if not config or not config.get('generated_samples'):
        ...
        if st.button("🎤 Ir a Audio Dataset Manager", type="primary"):
            st.switch_page("pages/01_🎵_Audio_Dataset.py")
    ...
    if palabra_selected and variacion_idx is not None:
        ...
        if analysis_result:
            ...
            if 'file_path' in variacion_data:
                repo_root = Path(__file__).parent.parent
                file_path = repo_root / variacion_data['file_path']
                if file_path.exists():
                    st.audio(str(file_path), format='audio/wav')
                else:
                    st.error(f"Archivo de audio no encontrado: {file_path}")
...
def main():
    ...
    display_modern_sidebar("audio_analytics")
