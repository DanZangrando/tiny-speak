...
def generar_variaciones_completas(texto, idioma, num_variaciones, metodo_sintesis='gtts'):
    """
    ...
    # Audio original
    audio_base64 = base64.b64encode(audio_base).decode('utf-8')
    ...
    for i in range(num_variaciones):
        ...
        if audio_variado:
            audio_var_base64 = base64.b64encode(audio_variado).decode('utf-8')
            duracion_var_ms = len(AudioSegment.from_wav(io.BytesIO(audio_variado)))
            
            resultado_variacion = {
                'audio_base64': audio_var_base64,
    ...
    if palabra_seleccionada and variacion_idx is not None:
        ...
        if 'audio_base64' in variacion_data:
            try:
                ...
                with col_btn1:
                    if st.button("📥 Descargar Audio", key=f"download_{palabra_seleccionada}_{variacion_idx}"):
                        st.download_button(
                            label="Descargar WAV",
                            data=audio_bytes,
