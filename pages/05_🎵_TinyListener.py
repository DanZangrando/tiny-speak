...
def _run_training_once(config: TrainingConfig) -> Dict:
    ...
    # Checkpointing to models/listener
    from pytorch_lightning.callbacks import ModelCheckpoint
    listener_models_dir = Path.cwd() / "models" / "listener"
    listener_models_dir.mkdir(parents=True, exist_ok=True)
    ckpt_cb = ModelCheckpoint(
        dirpath=str(listener_models_dir),
        filename="listener_{epoch:02d}_{val_top1:.2f}",
        monitor="val_top1",
        mode="max",
        save_top_k=2,
        save_last=True,
...
def render_training_tab(class_names: List[str]) -> None:
    ...
    if submitted:
        ...
        if result.get("best_checkpoint"):
            st.info(f"💾 Checkpoint guardado en: {result['best_checkpoint']}")
