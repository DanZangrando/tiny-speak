    ...
    try:
        if archivo_path.exists():
            with open(archivo_path, 'r', encoding='utf-8') as f:
                # Cargar palabras no vacías y asegurar unicidad preservando el orden
                raw_palabras = [line.strip() for line in f if line.strip()]
                palabras = list(dict.fromkeys(raw_palabras))
        else:
